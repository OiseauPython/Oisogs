  ...
  while (retryCount < maxRetries) {
    try {
      ...
      if (retryCount === maxRetries - 1) {
        const errorKey = { 403: 'private', 404: 'not_found', 429: 'rate_limit' }[error.message] || 'generic'
        throw { status: error.message, errorKey }
      }
      retryCount++
    }
  }
  ...
  async getAllCollection(username, onProgress) {
    ...
    while (hasNextPage) {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, data.pagination.pages > 29 ? 2000 : 0),
      )
    }
